<?php
    ...
    $search = isset($_POST["str"]) ? $f->checkString($_POST["str"]) : "";
    ...
    if($_POST["secret"] === "Wmfd2893gb7"){
        ...
        switch($type){
            ...
            case "6": // Featured
                ...
                $order = "rateDate DESC";
                break;
            case "7": // Magic (suggest levels)
                $query = $db->prepare("SELECT levelID FROM suggests ORDER BY suggestDate DESC LIMIT 10 OFFSET $offset");
                $query->execute();
                $suggestLevels = $query->fetchAll();
                $suggestLevelIDs = "";
                foreach($suggestLevels AS $suggestLevel) $suggestLevelIDs .= $suggestLevel["levelID"].', ';
                $suggestLevelIDs = substr($suggestLevelIDs, 0, -2);
                if($suggestLevelIDs != ""){
                    $where[] = "levelID IN ($suggestLevelIDs)";
                } else exit("##0:0:10");
                break;
            case "10": // Map Pack
                $mapPackLevels = explode(",", $search);
                if(count($mapPackLevels) == 3 AND is_numeric($mapPackLevels[0]) AND is_numeric($mapPackLevels[1]) AND is_numeric($mapPackLevels[2])){
                    $levelsCount = 0;

                    $query = $db->prepare("SELECT * FROM levels WHERE levelID = :levelID LIMIT 1");
                    $query->execute([':levelID' => $mapPackLevels[0]]);
                    if($query->rowCount() == 1){
                        $levels[] = $query->fetch();
                        $levelsCount++;
                    }
                    $query = $db->prepare("SELECT * FROM levels WHERE levelID = :levelID LIMIT 1");
                    $query->execute([':levelID' => $mapPackLevels[1]]);
                    if($query->rowCount() == 1){
                        $levels[] = $query->fetch();
                        $levelsCount++;
                    }
                    $query = $db->prepare("SELECT * FROM levels WHERE levelID = :levelID LIMIT 1");
                    $query->execute([':levelID' => $mapPackLevels[2]]);
                    if($query->rowCount() == 1){
                        $levels[] = $query->fetch();
                        $levelsCount++;
                    }
        ...
        if($type != 10){
            ...
            $query = $db->prepare("SELECT * FROM levels".$string." LIMIT 10 OFFSET $offset");
            $query->execute();
